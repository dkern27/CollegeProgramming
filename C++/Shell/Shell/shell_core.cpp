...
int Shell::loop_and_handle_input() {
  ...
  int return_value = 0;
  setenv("PATH", getenv("HOME"), 1); //Set starting directory as home
  while (true) {
...
string Shell::get_prompt(int return_value) {
    ...
    string cmdSuccessful = ":)";
    if(return_value != 0)
        cmdSuccessful = ":(";
    string prompt(username);
    prompt += " " + cmdSuccessful + " > ";
  return "\n" + prompt;
}
...
int Shell::execute_line(char* line) {
  ...
  // HINT: leverage readline! This should only be a couple lines of code.
    ...
    int expandSuccess = history_expand(line, &line);
    if (expandSuccess == 1)
    {
        cout << line << endl;
    }
...
void Shell::alias_substitution(vector<string>& tokens) {
    if(aliases.find(tokens[0]) != aliases.end())
    {
        tokens[0] = aliases.at(tokens[0]);
    }
}
